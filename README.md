...
> Изначально проект разрабатывался с использованием пакетного менеджера `yarn`, но можно и быстро через `npm` (не рекомендуется), если установка была не через `snap`.
> <details>
...
5. (если использовался `yarn`) Включить `corepack`.

`corepack enable` (или `npm install -g corepack` если старая версия `node`) \
Либо можно выполнить команду в "Вариант через `yarn` если необходимо" выше. \
При использовании `npm` в качестве пакетного менеджера эти действия выполнять не нужно.

6. (необязательно) Добавить alias для запуска CLI из любой папки
